...
use Filament\Infolists\Components\TextEntry; use Filament\Infolists\Components\Section as InfoSection; use Filament\Schemas\Components\Section; use Filament\Schemas\Schema;
class PembayaranInfolist { public static function configure(Schema $schema):Schema{return $schema->components([
 Section::make('Informasi Pembayaran')->description('Pembayaran menjadi sah setelah dikonfirmasi supplier.')->icon('heroicon-o-banknotes')->schema([
  ...
  TextEntry::make('tanggal_bayar')->label('Tanggal Pembayaran')->date('d M Y'), TextEntry::make('metode_pembayaran')->label('Metode Pembayaran')->badge(),
  TextEntry::make('total_bayar')->label('Jumlah Pembayaran')->money('IDR',locale:'id')->weight('bold'),
  TextEntry::make('status')->label('Status')->formatStateUsing(fn($state)=>match($state){'menunggu_supplier'=>'Menunggu Persetujuan Supplier','disetujui_supplier'=>'Disetujui Supplier','ditolak_supplier'=>'Ditolak Supplier',default=>ucfirst((string)$state)})->badge()->color(fn($state)=>match($state){'disetujui_supplier'=>'success','ditolak_supplier'=>'danger',default=>'warning'}),
